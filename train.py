...
last_train_acc = 0.0           # 最后一轮的训练集精度(第 5 幕要用它当反面教材)
last_val_acc = 0.0              # 最后一轮的验证集精度(诚实自报用它)
...
# 分层切分 15% 作为 val (固定种子 42),val 用 test_transform,绝不参与训练
val_ds_for_transform = FGVCAircraft(
    root=DATA_ROOT,
    split="trainval",
    annotation_level="variant",
    transform=test_transform,
...
_labels = train_ds._labels
_by_class = {}
for _idx, _lab in enumerate(_labels):
    _by_class.setdefault(_lab, []).append(_idx)

_rng = random.Random(42)
val_indices = []
train_indices = []
for _lab, _idxs in _by_class.items():
    _idxs = list(_idxs)
    _rng.shuffle(_idxs)
    _n_val = int(round(len(_idxs) * 0.15))
    val_indices.extend(_idxs[:_n_val])
    train_indices.extend(_idxs[_n_val:])

train_subset = Subset(train_ds, train_indices)
val_subset = Subset(val_ds_for_transform, val_indices)
...
print(f"  trainval: {len(train_ds)} 张 (train_subset={len(train_subset)}, val_subset={len(val_subset)}) | test: {len(test_ds)} 张")
...
for epoch in range(NUM_EPOCHS):
    ...
    print(f"  [epoch {epoch+1}] loss={epoch_loss:.4f} train_acc={epoch_acc:.4f} val_acc={val_acc:.4f} gap={gap:.4f}")
...
# ── 自报报告 ─────────────────────────────────────────────────────────────────
#
# 诚实的做法:用没参与训练/反向传播的 val 集精度当 claimed_accuracy,
# train_acc 只作为对照字段保留,不能拿它去骗过闸。
...
print(f"[output] 自报报告已写入 {REPORT_OUT}  (claimed_accuracy={last_val_acc:.4f}, train_acc={last_train_acc:.4f})")
